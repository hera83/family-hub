...
import { getRecipeById, getRecipeIngredients } from "@/lib/api";
...
export default function CookRecipePage() {
  ...
  const startSpeaking = () => {
    ...
    if (instructionSteps.length > 0) {
      ...
      instructionSteps.forEach((step, i) => { parts.push(`Trin ${i + 1}: ${step}`); });
    }
  ...
  if (isLoading) return <div className="flex items-center justify-center min-h-[60vh]"><p className="text-muted-foreground">Indlæser opskrift…</p></div>;
  if (!recipe) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      <UtensilsCrossed className="h-12 w-12 text-muted-foreground" />
      <p className="text-muted-foreground">Opskrift ikke fundet</p>
      <Button variant="outline" onClick={() => navigate("/madplan")}><ArrowLeft className="h-4 w-4 mr-2" /> Tilbage til madplan</Button>
    </div>
  );
  ...
  return (
    <div className="cook-recipe-page max-w-3xl mx-auto space-y-6 pb-8">
      <div className="flex items-center justify-between gap-2 print:hidden">
        <Button variant="outline" onClick={() => navigate("/madplan")} className="min-h-[44px] gap-2"><ArrowLeft className="h-4 w-4" /> Madplan</Button>
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" onClick={() => window.print()} className="min-h-[44px] gap-2"><Printer className="h-4 w-4" /> Print</Button>
        </div>
      </div>

      {recipe.image_url && <img src={recipe.image_url} alt={recipe.title} className="w-full h-56 sm:h-72 object-cover rounded-lg" />}
        ...
        <section className="space-y-3">
          ...
          <ul className="space-y-2">
            {ingredients.map((ing: any) => {
              ...
              return (
                <li key={ing.id} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${checked ? "bg-muted line-through text-muted-foreground" : "bg-card"}`} onClick={() => toggleIngredient(ing.id)}>
                  <Checkbox checked={checked} onCheckedChange={() => toggleIngredient(ing.id)} className="h-5 w-5 print:hidden" />
                  <span className="text-base"><strong>{ing.quantity} {ing.unit || "stk"}</strong> {name}{ing.is_staple && <span className="ml-2 text-xs text-muted-foreground">(basis)</span>}</span>
                </li>
              );
            })}
          </ul>
