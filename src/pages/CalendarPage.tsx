...
import { getFamilyMembers, getNormalEvents, getRecurringEvents, upsertCalendarEvent, deleteCalendarEvent } from "@/lib/api";
import { Button } from "@/components/ui/button";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, DialogDescription } from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Checkbox } from "@/components/ui/checkbox";
import { ChevronLeft, ChevronRight, Plus, Trash2, Pencil, Repeat } from "lucide-react";
import {
  format, startOfWeek, endOfWeek, addWeeks, subWeeks, addMonths, subMonths,
  eachDayOfInterval, startOfMonth, endOfMonth, isSameDay, isSameMonth, isToday,
  getDay, getDate, getMonth, isBefore, parseISO,
} from "date-fns";
import { da } from "date-fns/locale";

const DAY_LABELS = ["Man", "Tir", "Ons", "Tor", "Fre", "Lør", "Søn"];
const DAY_OPTIONS = [
  { value: 1, label: "Mandag" }, { value: 2, label: "Tirsdag" }, { value: 3, label: "Onsdag" },
  { value: 4, label: "Torsdag" }, { value: 5, label: "Fredag" }, { value: 6, label: "Lørdag" }, { value: 7, label: "Søndag" },
];

function toISODay(jsDay: number) { return jsDay === 0 ? 7 : jsDay; }
...
function recursFallsOnDay(event: any, day: Date): boolean {
  ...
  switch (event.recurrence_type) {
    case "weekly": return ((event.recurrence_days as number[]) || []).includes(toISODay(getDay(day)));
    case "monthly": return getDate(day) === getDate(eventStart);
    case "yearly": return getDate(day) === getDate(eventStart) && getMonth(day) === getMonth(eventStart);
...
export default function CalendarPage() {
  ...
  const { data: normalEvents = [] } = useQuery({
    ...
    queryFn: () => getNormalEvents(format(dateRange.days[0], "yyyy-MM-dd"), format(dateRange.days[dateRange.days.length - 1], "yyyy-MM-dd")),
  });

  const { data: recurringEvents = [] } = useQuery({
    queryKey: ["recurring_events", dateRange.days[dateRange.days.length - 1].toISOString()],
    queryFn: () => getRecurringEvents(format(dateRange.days[dateRange.days.length - 1], "yyyy-MM-dd")),
  });
  ...
  const openAddEvent = (date?: Date) => {
    ...
    setNewEvent({
      ...
      start_time: "", end_time: "", member_id: members[0]?.id || "", recurrence_type: "", recurrence_days: [],
    });
  ...
  const openEditEvent = (event: any) => {
    ...
    setNewEvent({
      ...
      start_time: event.start_time || "", end_time: event.end_time || "", member_id: event.member_id || "",
  ...
  const getEventsForDay = (day: Date) => {
    ...
    const recurring = recurringEvents.filter((e: any) => recursFallsOnDay(e, day)).map((e: any) => ({ ...e, _virtualDate: format(day, "yyyy-MM-dd") }));
    return [...normal, ...recurring];
  };

  const getMemberColor = (memberId: string) => members.find((m: any) => m.id === memberId)?.color || "hsl(210, 15%, 70%)";
  const getMemberName = (memberId: string) => members.find((m: any) => m.id === memberId)?.name || "Ukendt";
  ...
  const toggleRecurrenceDay = (day: number) => {
    setNewEvent(prev => ({
      ...prev,
      recurrence_days: prev.recurrence_days.includes(day) ? prev.recurrence_days.filter(d => d !== day) : [...prev.recurrence_days, day],
    }));
  };
  ...
  const weekActivityData = useMemo(() => {
    ...
    dateRange.days.forEach((day) => {
      ...
      dayEvents.forEach((e: any) => {
        ...
        if (!memberMap[mid]) {
          memberMap[mid] = { memberId: mid, name: getMemberName(mid), color: getMemberColor(mid), activities: dateRange.days.map(d => ({ day: d, events: [] })) };
        }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <Button variant="outline" size="icon" onClick={() => navigate("prev")} className="min-h-[44px] min-w-[44px]"><ChevronLeft className="h-5 w-5" /></Button>
          ...
          <Button variant="outline" size="icon" onClick={() => navigate("next")} className="min-h-[44px] min-w-[44px]"><ChevronRight className="h-5 w-5" /></Button>
        </div>
        <div className="flex items-center gap-2">
          <Button variant={viewMode === "week" ? "default" : "outline"} onClick={() => setViewMode("week")} className="min-h-[44px]">Uge</Button>
          <Button variant={viewMode === "month" ? "default" : "outline"} onClick={() => setViewMode("month")} className="min-h-[44px]">Måned</Button>
          <Button size="icon" onClick={() => openAddEvent()} className="min-h-[44px] min-w-[44px]"><Plus className="h-5 w-5" /></Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {DAY_LABELS.map((d, i) => (
          <div key={d} className={`text-center text-xs font-medium py-2 ${i >= 5 ? "text-weekend-foreground" : "text-muted-foreground"}`}>{d}</div>
        ))}
        {dateRange.days.map((day) => {
          ...
          return (
            ...
            >
              <div className={`text-xs font-medium mb-1 ${isToday(day) ? "text-primary" : "text-muted-foreground"}`}>{format(day, "d")}</div>
              <div className="space-y-0.5">
                {viewMode === "week"
                  ? dayEvents.slice(0, 4).map((e: any, i: number) => (
                      <div key={e.id + (e._virtualDate || "") + i} className="text-xs px-1.5 py-0.5 rounded truncate cursor-pointer flex items-center gap-0.5" style={{ backgroundColor: getMemberColor(e.member_id), color: "white" }} onClick={(ev) => { ev.stopPropagation(); openEditEvent(e); }}>
                  ...
                  : (() => {
                      const grouped = dayEvents.reduce((acc: any, e: any) => { const mid = e.member_id || "none"; if (!acc[mid]) acc[mid] = []; acc[mid].push(e); return acc; }, {});
                      return Object.entries(grouped).slice(0, 3).map(([mid, evts]: any) => (
                        <div key={mid} className="text-xs px-1 py-0.5 rounded cursor-pointer flex items-center gap-1" style={{ backgroundColor: getMemberColor(mid), color: "white" }} onClick={(ev) => { ev.stopPropagation(); setShowEventPopup({ date: day, memberId: mid }); }}>
                          <span className="truncate">{evts.length} aktivitet{evts.length > 1 ? "er" : ""}</span>
                        </div>
                      ));
                    })()}
                {dayEvents.length > 4 && viewMode === "week" && <div className="text-xs text-muted-foreground px-1">+{dayEvents.length - 4} mere</div>}
              </div>
            </div>
          );
        })}
                    ...
                    <td key={dayIdx} className="p-1 align-top">
                      <div className="space-y-0.5">
                        {act.events.map((e: any, i: number) => (
                          <div key={e.id + i} className="text-xs px-1.5 py-0.5 rounded cursor-pointer truncate hover:opacity-80" style={{ backgroundColor: member.color, color: "white" }} onClick={() => openEditEvent(e)}>
      ...
      <Dialog open={showEventDialog} onOpenChange={setShowEventDialog}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            ...
            <div>
              <Label>Medlem</Label>
              <select value={newEvent.member_id} onChange={(e) => setNewEvent({ ...newEvent, member_id: e.target.value })} className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
            ...
            <div className="border-t pt-3 space-y-3">
              <div className="flex items-center gap-2"><Repeat className="h-4 w-4 text-muted-foreground" /><Label className="text-sm font-medium">Gentagelse</Label></div>
              <select value={newEvent.recurrence_type} onChange={(e) => setNewEvent({ ...newEvent, recurrence_type: e.target.value as any, recurrence_days: [] })} className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
              ...
              {newEvent.recurrence_type === "weekly" && (
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {DAY_OPTIONS.map((d) => (
                      <button key={d.value} type="button" onClick={() => toggleRecurrenceDay(d.value)} className={`px-3 py-2 rounded-md text-sm font-medium min-h-[44px] transition-colors border ${newEvent.recurrence_days.includes(d.value) ? "bg-primary text-primary-foreground border-primary" : "bg-background text-foreground border-input hover:bg-muted"}`}>
                        {d.label.slice(0, 3)}
                      </button>
                    ))}
                  </div>
              ...
              {newEvent.recurrence_type === "monthly" && <p className="text-xs text-muted-foreground">Gentages den {newEvent.event_date ? format(parseISO(newEvent.event_date), "d.") : "valgte dato"} hver måned</p>}
              {newEvent.recurrence_type === "yearly" && <p className="text-xs text-muted-foreground">Gentages den {newEvent.event_date ? format(parseISO(newEvent.event_date), "d. MMMM", { locale: da }) : "valgte dato"} hvert år</p>}
            </div>
          </div>
          <DialogFooter className="gap-2">
            ...
            <Button onClick={() => {
              const payload: any = {
                title: newEvent.title, description: newEvent.description || null, event_date: newEvent.event_date,
                start_time: newEvent.start_time || null, end_time: newEvent.end_time || null, member_id: newEvent.member_id || null,
                recurrence_type: newEvent.recurrence_type || null,
                recurrence_days: newEvent.recurrence_type === "weekly" && newEvent.recurrence_days.length > 0 ? newEvent.recurrence_days : null,
              };
              if (editingEvent) payload.id = editingEvent.id;
              addEvent.mutate(payload);
            }} disabled={!newEvent.title || !newEvent.event_date || (newEvent.recurrence_type === "weekly" && newEvent.recurrence_days.length === 0)} className="min-h-[44px]">
      ...
      <Dialog open={!!showEventPopup} onOpenChange={() => setShowEventPopup(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>{showEventPopup && format(showEventPopup.date, "d. MMMM", { locale: da })} – {members.find((m: any) => m.id === showEventPopup?.memberId)?.name}</DialogTitle>
            <DialogDescription>Aktiviteter for dette medlem på denne dag</DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            {showEventPopup && getEventsForDay(showEventPopup.date).filter((e: any) => e.member_id === showEventPopup.memberId).map((e: any, i: number) => (
              <div key={e.id + i} className="flex items-center justify-between p-2 rounded-md bg-muted">
                <div>
                  <div className="font-medium text-sm flex items-center gap-1">{e.recurrence_type && <Repeat className="h-3 w-3" />}{e.title}</div>
